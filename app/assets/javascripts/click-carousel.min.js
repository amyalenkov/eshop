$(function(){
    $("#containerSlider").clickCarousel({margin: 10});
});
...
(function(jQuery) {
    ...
    jQuery.fn.fakeFloat = function(options, callback)
    {
        ...
        jQuery(this).each(function()
        {
            ...
            {
                jQuery(this).animate({"left": ((settings.margin) + 190)*i + (settings.offset) + 'px'}, settings.speed);
            }
            else
            {
                // 190 - для Other_products in show product
                jQuery(this).animate({"right": ((settings.margin) + 190)*i + (settings.offset) + 'px'}, settings.speed);
            }
            i++;
        });
